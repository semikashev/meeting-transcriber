...
- **Calendar titles** — Opt-in (Settings → General): a recording is named after the calendar event running when it starts, and the event's attendees are handed to the protocol. Reads the calendars already on the Mac via EventKit; a recording that starts after an event ended is never filed under it. A second opt-in under it records a browser meeting without the consent prompt when the call falls into an event with other attendees or a conference link — calls outside the calendar and personal blocks still ask, and "Never for this app" wins. A third opt-in offers to record in-person meetings: when an event with other attendees but no call link begins while watching is on and nothing is recording, a notification asks whether to record the microphone; Record starts a microphone recording named after the event. Watching that was on resumes after any manual recording, from the prompt or the menu
